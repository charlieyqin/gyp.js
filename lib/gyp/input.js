...
function loadOneBuildFile(buildFilePath, data, auxData, includes, isTarget,
  ...
  try {
    if (check)
      buildFileData = gyp.py.checkedParseJSON(buildFileContents, buildFilePath);
    else
  ...
  if (!buildFileData['skip_includes']) {
    try {
      loadBuildFileIncludesIntoDict(buildFileData, buildFilePath, data,
                                    auxData, isTarget ? includes : undefined,
                                    check);
    } catch (e) {
...
function expandVariables(input, phase, variables, buildFile) {
  ...
  for (let i = matches.length - 1; i >= 0; i--) {
    ...
    const cGroup = findEnclosingBracketGroup(inputStr.slice(replaceStart));
    const cStart = cGroup[0], cEnd = cGroup[1];
  ...
  Object.keys(targets).forEach((target) => {
    ...
    allDependencySections.forEach((dependencyKey) => {
      ...
      for (let index = 0; index < dependencies.length; index++) {
        ...
        //let [ depFile, depTarget, depToolset ] = 

        // Ignore toolset specification in the dependency if it is specified.
        if (!multipleToolsets)
          resolved.toolset = toolset;
        const dependency =
            gyp.common.qualifiedTarget(resolved.buildFile, resolved.target, resolved.toolset);
  ...
  Object.keys(targets).forEach((target) => {
    ...
    dependencySections.forEach((dependencyKey) => {
      ...
      while (index < dependencies.length) {
        const dependency = gyp.common.parseQualifiedTarget(dependencies[index]);
        if (dependency.target !== '*' && dependency.toolset !== '*') {
          // Not a wildcard.  Keep it moving.
          ++index;
          continue;
        }

        if (dependency.buildFile === targetBuildFile) {
        ...
        // wildcard.
        const dependencyTargetDicts = data[dependency.buildFile]['targets'];
        dependencyTargetDicts.forEach((dependencyTargetDict) => {
          ...
          if (dependency.target !== '*' &&
              dependency.target !== dependencyTargetName) {
            return;
          }
          const dependencyTargetToolset = dependencyTargetDict['toolset'];
          if (dependency.toolset !== '*' &&
              dependency.toolset !== dependencyTargetToolset) {
            return;
          }
          dependencies.splice(++index, 0, gyp.common.qualifiedTarget(
            dependency.buildFile, dependency.target, dependency.toolset));
        });
        ++index;
      }
    });
...
DependencyGraphNode.prototype.findCycles = function findCycles() {
  ...
  function visit(node, path) {
    node.dependents.forEach((child) => {
      ...
      if (i !== -1) {
        results.push([ child ].concat(path.slice(0, index + 1)));
      } else {
        visited.add(child);
...
DependencyGraphNode.prototype.deepDependencies =
  ...
  this.dependencies.forEach((dependency) => {
    ...
    if (dependencies.has(dependency.ref) === -1) {
...
function verifyNoGYPFileCircularDependencies(targets) {
  ...
  const rootNode = new DependencyGraphNode(undefined);
  dependencyNodes.forEach((buildFileNode, k) => {
...
function mergeLists(to, fro, toFile, froFile, isPaths, append) {
  if (isPaths === undefined) isPaths = false;
...
function mergeConfigWithInheritance(newConfigurationDict, buildFile,
  ...
  (configurationDict['inheritFrom'] || []).forEach((parent) => {
    mergeConfigWithInheritance(newConfigurationDict, buildFile,
                               targetDict, parent,
...
function setUpConfigurations(target, targetDict) {
  ...
  Object.keys(configs).forEach((configuration) => {
    ...
    mergeConfigWithInheritance(newConfigurationDict, buildFile, targetDict,
                               configuration, []);
  ...
  Object.keys(targetDict).forEach((key) => {
    ...
    if (keySuffixes.test(keyExt))
      ...
      keyBase = key;
    if (nonConfigurationKeys.indexOf(key) === -1)
      deleteKeys.push(key);
  });
...
function validateSourcesInTarget(target, targetDict, buildFile,
                                 duplicateBasenameCheck) {
  if (!duplicateBasenameCheck)
    return;
  if (targetDict['type'] !== 'static_library')
    return;

  const sources = targetDict['sources'] || [];
  const basenames = {};
  sources.forEach((source) => {
    // PORT: os.path.splitext
    const split = gyp.common.splitext(source);
    const name = split[0], ext = split[1];

    const isCompiledFile = /\.(c|cc|cpp|cxx|m|mm|s|S)$/.test(ext);
    if (!isCompiledFile)
      return;

    const basename = path.basename(name);  // Don't include extension
    if (!basenames[basename])
      basenames[basename] = [];
    basenames[basename].push(source);
  });
  ...
  Object.keys(basenames).forEach((basename) => {
    ...
    if (files.length > 1)
      error += ` ${basename}: ${files.join(' ')}`;
  });

  if (error) {
    console.log(
      `static library ${target} has several files with the same basename:\n` +
      error +
      `libtool on Mac cannot handle that. Use --no-duplicate-basename-check ` +
      `to disable this validation.`);
    throw new Error('Duplicate basenames in sources section, see list above');
  }
}
  ...
  rules.forEach((rule) => {
    ...
    sourceKeys.forEach((sourceKey) => {
      (targetDict[sourceKey] || []).forEach((source) => {
        // PORT: os.path.splitext
        const split = gyp.common.splitext(source);
        const sourceRoot = split[0], sourceExtension = split[1];
...
function validateActionsInTarget(target, targetDict, buildFile) {
...
function pruneUnwantedTargets(targets, flatList, dependencyNodes, rootTargets,
  ...
  const wantedFlatList = flatList.filter(t => wantedTargets[t] !== undefined);
  ...
  return [ wantedTargets, wantedFlatList ];
}
...
function verifyNoCollidingTargets(targets) {
  ...
  targets.forEach((target) => {
    ...
    const tpath = match[1], name = match[2];
...
function load(buildFiles, variables, includes, depth, generatorInputInfo, check,
              circularCheck, duplicateBasenameCheck, rootTargets) {
  ...
  buildFiles.forEach((buildFile) => {
    try {
      loadTargetBuildFile(buildFile, data, auxData, variables, includes, depth,
                          check, true);
    } catch (e) {
  ...
  const targets = buildTargetsDict(data);
  ...
  Object.keys(targets).forEach((targetName) => {
    ...
    processListFiltersInDict(targetName, tmpDict);
  ...
  const depsList = buildDependencyList(targets);
  const dependencyNodes = depsList[0], flatList = depsList[1];

  if (rootTargets && rootTargets.length !== 0) {
    // Remove, from |targets| and |flat_list|, the targets that are not deep
    // dependencies of the targets specified in |root_targets|.
    const pruned = pruneUnwantedTargets(
        targets, flatList, dependencyNodes, rootTargets, data);
    targets = pruned[0], flatList = pruned[1];
  }

  // Check that no two targets in the same directory have the same name.
  verifyNoCollidingTargets(flatList);

  // Handle dependent settings of various types.
  [ 'all_dependent_settings',
    'direct_dependent_settings',
    'link_settings' ].forEach((settingsType) => {
    doDependentSettings(settingsType, flatList, targets, dependencyNodes);
  ...
  if (gii['generator_wants_static_library_dependencies_adjusted']) {
    adjustStaticLibraryDependencies(flatList, targets, dependencyNodes,
  ...
  flatList.forEach((target) => {
    const targetDict = targets[target];
    setUpConfigurations(target, targetDict);
  });

  // Apply exclude (!) and regex (/) list filters.
  flatList.forEach((target) => {
    const targetDict = targets[target];
    processListFiltersInDict(target, targetDict);
  });
  ...
  flatList.forEach((target) => {
    ...
    validateSourcesInTarget(target, targetDict, buildFile,
                            duplicateBasenameCheck);
  ...
  // TODO(indutny): this should be an object
  return { flatList, targets, data };
}
exports.load = load;
