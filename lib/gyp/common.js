...
function relativePath(tpath, relativeTo, followPathSymlink) {
  if (followPathSymlink === undefined) followPathSymlink = true
  ...
  if (relativeSplit.length === 0)
    ...
    res = path.join.apply(path, relativeSplit);
...
exports.resolveTarget = function resolveTarget(buildFile, target, toolset) {
  ...
  if (parsed.buildFile) {
    if (buildFile) {
      ...
      if (path.isAbsolute(parsed.buildFile))
        buildFile = path.relative('.', parsed.buildFile);
      else
        buildFile = path.normalize(path.join(dir, parsed.buildFile));
      // Further (to handle cases like ../cwd), make it relative to cwd)
      // PORT: RelativePath()
      if (!path.isAbsolute(buildFile))
        buildFile = relativePath(buildFile, '.');
    } else {
      buildFile = parsed.buildFile;
    }
  }
...
exports.cachedRelative = function cachedRelative(from, to) {
  ...
  if (RELATIVE_CACHE.has(key))
    return path.join(RELATIVE_CACHE.get(key), basename);

  const value = path.relative(from, dirname);
  RELATIVE_CACHE.set(key, value);
  return path.join(value, basename);
};
