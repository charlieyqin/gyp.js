...
Condition.prototype.parseOne = function parseOne(priority) {
  ...
  while (!this.ended()) {
    ...
    seed = { type: 'Binary', op: parsed.op, left: seed, right: right };
  }
...
Condition.prototype.parseUnary = function parseUnary(priority) {
  // <op> exp
  const parsed = this.parseOp(priority);
  if (!parsed.op || parsed.op !== 'not')
    return this.parseIdOrLiteral();

  this.space();
  return { type: 'Unary', op: parsed.op, argument: this.parseOne(parsed.nextPriority) };
};
