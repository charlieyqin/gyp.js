...
function calculateVariables(defaultVariables, params) {
  ...
  if (process.platform === 'darwin') {
    ...
    // TODO(indutny): MSVS_VERSION, MSVS_OS_BITS
    setdef('OS', 'win')
...
function Ninja(options) {
  //options = { index, outDir, targetDict, ninjas, config }
  const parsed = gyp.common.parseQualifiedTarget(options.target);

  let obj = 'obj';
  if (parsed.toolset !== 'target')
    obj += '.' + parsed.toolset;
  ...
  this.intDir = path.dirname(parsed.buildFile);
  this.objDir = path.join(options.outDir, obj, this.intDir);
  this.srcDir = path.dirname(parsed.buildFile);
  this.useCxx = false;

  const filename = path.join(this.objDir, parsed.target) + '.ninja';
...
Ninja.prototype.expand = function expand(p, productDir) {
  if (productDir === undefined) productDir = '.'; 
  if (productDir === '.')
...
NinjaMain.prototype.rulesAndTargets = function rulesAndTargets() {
  ...
  if (process.platform === 'win32') {
    gyp.platform.win.ninjaRules(main);
  } else {
...
NinjaMain.prototype.defaults = function defaults() {
  ...
  this.params.build_files.forEach((buildFile) => {
    this.targetList.forEach((target) => {
      const parsed = gyp.common.parseQualifiedTarget(target);
      if (parsed.buildFile !== buildFile)
        return;
