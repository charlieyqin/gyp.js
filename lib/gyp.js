...
function load(buildFiles, format, defaultVariables, includes,
              depth, params, check, circularCheck, duplicateBasenameCheck) {
  if (defaultVariables === undefined) defaultVariables = {};
  if (includes === undefined) includes = [];
  if (!depth) depth = '.';
  if (params == undefined) params = {};
  if (check === undefined) check = false;
  if (circularCheck === undefined) circularCheck = true;
  if (duplicateBasenameCheck === undefined) duplicateBasenameCheck = true;
  /*
  ...
  const result = gyp.input.load(
      ...
      params['root_targets']);
  return Object.assign(generator, result);
}
...
gyp.main = function main(args) {
  ...
  const generator = load(buildFiles, 'ninja',
   ...
   */
  generator.generateOutput(generator.flatList, generator.targets, generator.data, params);
  ...
  if (options.configs.length !== 0) {
    ...
    generator.performBuild(generator.data, options.configs, params);
  }
